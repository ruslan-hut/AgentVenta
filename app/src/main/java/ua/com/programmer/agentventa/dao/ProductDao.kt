...
interface ProductDao {
    ...
    @Query("SELECT " +
            ...
            "product.indivisible," +
            "CASE WHEN doc_order.price > 0 THEN doc_order.price WHEN prices.price > 0 THEN prices.price ELSE product.price END AS price," +
            "product.weight," +
            ...
            "WHERE " +
            "CASE :filter WHEN '' THEN product.group_guid=:group " +
            "ELSE CASE :group WHEN '' THEN is_group=0 AND (product.description LIKE :filter OR product.code1 LIKE :filter) " +
            "ELSE product.group_guid=:group AND (product.description LIKE :filter OR product.code1 LIKE :filter) END END " +
            "AND product.db_guid=:dbGuid " +
            "AND CASE :restOnly WHEN 1 THEN product.quantity>0 OR doc_order.quantity>0 OR product.is_group=1 ELSE 1=1 END " +
            "ORDER BY :sorting")
    fun getProducts(dbGuid: String, filter: String, group: String, order: String, restOnly: Int, sorting: String, type: String): Flow<List<LProduct>>
    ...
    @Query("SELECT " +
            ...
            "LEFT OUTER JOIN (SELECT product_guid, url, guid, db_guid FROM product_images WHERE isDefault=1) AS images " +
            "ON product.guid=images.product_guid AND product.db_guid=images.db_guid " +
            "WHERE product.guid=:guid AND product.db_guid IN (SELECT guid FROM user_accounts WHERE is_current=1) ")
    ...
    @Query("SELECT " +
            ...
            "ON prices.price_type=types.price_type AND prices.db_guid=types.db_guid " +
            "LEFT OUTER JOIN (SELECT guid, base_price, db_guid FROM products WHERE guid=:productGuid) AS product " +
            "ON prices.product_guid=product.guid AND prices.db_guid=product.db_guid " +
            " WHERE prices.product_guid=:productGuid AND prices.db_guid IN (SELECT guid FROM user_accounts WHERE is_current=1)")
